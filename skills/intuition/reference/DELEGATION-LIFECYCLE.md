...
3. [Critical Encoding Rules (Read This First)](#3-critical-encoding-rules-read-this-first)
...
| **Private keys** | Main Account key (session-only, never persisted). OWS key (generated by Agent, SAVED as Agent's key for creation track) |
...
## 3. Critical Encoding Rules (Read This First)

The Intuition Delegation Framework uses the standard MetaMask Delegation Framework v1.3.0 patterns. Getting any of these wrong causes silent failures that are hard to debug.

### Rule 1: Struct Field Order (delegate FIRST)
...
### Rule 2: execCallData Encoding (solidityPacked)

**Standard:** `solidityPacked(['address','uint256','bytes'], [target, value, calldata])`

The `ExecutionLib.decodeSingle()` reads the inner calldata from byte offset **0x34** of execCallData. `solidityPacked` places it there. `abi.encode` adds ABI offset headers and left-padding, placing the inner calldata at byte **0x80+**, so `decodeSingle` reads garbage.

**Wrong encoding causes:** `AllowedMethodsEnforcer:method-not-allowed` (0x08c379a0) at gas 76543

### Rule 3: Salt is uint256, Not bytes32

The on-chain `Delegation` struct uses `uint256 salt`. Passing `bytes32 salt` causes `abi.decode` failures.
...
// ── ENCODING RULES (standard MetaMask pattern) ──
...
const delegation = {
  delegate: DELEGATE,      // FIRST (standard MetaMask order)
...
export DELEGATOR_KEY="0x<YOUR_TESTNET_KEY_HERE>"
